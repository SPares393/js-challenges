...
export const removeFalseValues = (booleanArr) => {
  const trueArr = booleanArr.filter((value) => value === true);
  return trueArr;
};
...
export const createPercentageList = (numbersArr) => {
  const percentageArr = numbersArr.map((number) => `${number * 100}%`);
...
export const createListOfPoessessions = (possessionsArr, name) => {
  const newArr = [];
  possessionsArr.forEach((possession) => {
    newArr.push(`${name} ${possession}`);
  });
  return newArr;
};
...
export const convertStringToNumbersArray = (numberString) => {
  ...
  numberArr = numberArr.map((number) => parseInt(number));
  return numberArr;
};
...
export const createOddEvenArray = (numberString) => {
  let splitArr = numberString.split("+");
  let oddEvenArr = [];
...
export const filterBooksBySearch = (booksArr, searchTerm) => {
  const newArr = booksArr.filter((book) => book.includes(searchTerm));
  return newArr;
};
